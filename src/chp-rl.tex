...
\newcommand{\wvec}{w}
\newcommand{\wvector}[1]{\ensuremath{\wvec_{#1}}}
...
the \emph{eligibility trace} $\et{t} \in \reals^d$
that parallels the long-term weight vector $\wvector{t} \in \reals^d$.
...
If the episode termintes at $t=T$,
...
\right) \nabla_\wvec \valappr(\state{t}, \wvector{t})
\end{equation}
for $t=0,\ldots, T-1$.
...
The \emph{eligibility trace} is a vector $\et{t} \in \reals^d$
...
which typically lasts less time than the length of an episode.
